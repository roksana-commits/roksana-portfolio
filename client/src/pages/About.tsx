  ...
  return (
    <div className="about-page">
      ...
      <main>
        ...
        <section className="about-journey-section"><div className="section-wrap"><div className="about-section-intro"><span className="section-label"><span>02</span><span>Freelancing origin</span></span><h2>Started with<br /><em>useful links.</em></h2><p className="about-client-note">The first milestone in a 9+ year practice built around research, publishing, and making information easier to act on.</p></div><div className="about-milestone-list">{journeyMilestones.map((milestone) => <article className="about-milestone" key={milestone.period}><span className="milestone-node" aria-hidden="true" /><div className="milestone-period mono">{milestone.period}</div><div className="milestone-body"><span className="mono">{milestone.company}</span><h3>{milestone.title}</h3><p>{milestone.detail}</p><ul>{milestone.responsibilities.map((responsibility) => <li key={responsibility}><Check size={13} /> {responsibility}</li>)}</ul></div></article>)}</div></div></section>
